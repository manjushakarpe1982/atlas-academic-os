  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-5 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-extrabold text-gray-900">Connected Calendar</p>
                <p className="text-xs text-gray-400 mt-0.5">Last synced · 2 mins ago 🟢</p>
              </div>
            </div>
            <span className="text-xs font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full">Active</span>
          </div>
          <button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 rounded-xl text-sm transition-all">
            Resync Now
          </button>
        </div>
        ...
        {/* Calendar Feed URL */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <p className="text-xs font-extrabold text-gray-400 uppercase tracking-widest mb-3">Calendar Feed URL</p>
          <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
            <span className="flex-1 text-xs text-gray-600 truncate font-mono">
              {showUrl ? MOCK_URL : '•'.repeat(42)}
            </span>
            <button onClick={() => setShowUrl(!showUrl)} className="text-gray-400 hover:text-gray-600 flex-shrink-0">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-50">
            <p className="text-xs font-extrabold text-gray-400 uppercase tracking-widest mb-2">Sync Frequency</p>
            <select value={freq} onChange={e => setFreq(e.target.value)}
              className="w-full text-sm font-semibold text-gray-900 outline-none bg-transparent">
              {['Every 15 minutes','Every 30 minutes','Every hour','Every day'].map(f => (
        ...
        {/* Disconnect */}
        <button className="w-full bg-red-50 text-red-600 font-bold py-3.5 rounded-2xl text-sm border border-red-100 hover:bg-red-100 transition-all">
          Disconnect Calendar
        </button>

        <button onClick={() => router.push('/calendar')}
          className="w-full border-2 border-indigo-200 text-indigo-600 font-bold py-3 rounded-2xl text-sm hover:bg-indigo-50 transition-all">
