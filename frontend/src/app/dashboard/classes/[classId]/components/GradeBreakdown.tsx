...
interface WeightItem { label: string; pct: number; color: string; }
interface Props { weights: WeightItem[]; currentGrade: number | null; }
            ...
            <svg viewBox="0 0 100 100" className="w-full h-full">
              {(() => {
                ...
                return weights.map(w => {
                  const a = w.pct / 100 * 360;
                  const r = 38;
                  const s = o * Math.PI / 180;
                  const e = (o + a) * Math.PI / 180;
                  ...
                  const l = a > 180 ? 1 : 0;
                  o += a;
                  return <path key={w.label} d={`M50 50 L${x1} ${y1} A${r} ${r} 0 ${l} 1 ${x2} ${y2}Z`} fill={w.color} />;
                });
              })()}
