...
export default function Screen3({ onNext, onBack, classId }: Props) {
  ...
  const uploadFile = async (file: File) => {
    if (!classId) {
      setError("No class ID. Go back and try again.");
      return;
    }
    setUploading(true);
    setError("");
  ...
  const UPLOAD_OPTIONS = [
    {
      icon: Camera,
      label: "Camera",
      sub: "Take a photo of your syllabus",
      accept: "image/*",
      ...
      <div className="flex flex-col bg-white overflow-y-auto">
        ...
        <div className=" mb-3">
          {uploading ? (
            ...
            <label className="block border-2 border-dashed border-indigo-200 rounded-xl p-6 py-12 text-center bg-indigo-50/40 hover:border-indigo-400 hover:bg-indigo-50 transition-all cursor-pointer">
              ...
              <p className="text-base font-extrabold text-gray-800 mb-0.5">
        ...
        <div className=" grid grid-cols-3 mt-3 gap-3 mb-3">
          {UPLOAD_OPTIONS.map((o) => (
            <label
              key={o.label}
              className="flex flex-col bg-white border border-indigo-100 rounded-xl px-1 py-3 items-center gap-1 cursor-pointer group"
            >
