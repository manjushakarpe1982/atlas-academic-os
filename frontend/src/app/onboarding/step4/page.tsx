...
import {
  Sliders, Clock, Zap, Target, Moon, Upload,
  Plus, X, Sunrise, Star,
} from 'lucide-react';
...
function DayPill({ d, active, onClick }: {
  d: string; active: boolean; onClick: () => void;
}) {
  return (
    <button type="button" onClick={onClick}
      className={`w-9 h-9 rounded-xl text-xs font-bold transition-all ${
        active
          ...
          : 'bg-[#f5f3ff] text-[#534AB7] hover:bg-[#ede9ff]'
      }`}>
      {d[0]}
    </button>
  );
  ...
  return (
    <OnboardingLayout step={4}>
      <div className="flex-1 flex flex-col">
        ...
        {/* Heading */}
        <div className="mb-5">
          <span className="inline-flex items-center gap-1.5 bg-[#534AB7]/10 text-[#534AB7] text-xs font-bold px-3 py-1 rounded-full mb-4">
            <Sliders className="w-3.5 h-3.5" /> Step 4 of 5
          </span>
          <h1 className="text-3xl font-extrabold text-[#1A1A2E] mb-2">Preferences</h1>
          <p className="text-[#6B6A8A] text-sm font-light">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-1">
          ...
          <div className="bg-white border border-[#e8e7f5] rounded-2xl p-4 shadow-sm">
              ...
              {TIME_OPTIONS.map((o) => (
                <button key={o.id} type="button" onClick={() => setStudyTime(o.id)}
                  className={`flex items-center gap-2.5 p-3 rounded-xl border-2 transition-all text-left ${
                    studyTime === o.id
                      ? `${o.ring} bg-[#f5f3ff]`
                      : 'border-[#e8e7f5] bg-[#fafafe] hover:border-[#534AB7]/30'
                  }`}>
                  ...
                  {studyTime === o.id && (
                    <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#534AB7] flex-shrink-0" />
                  )}
                </button>
              ))}
            </div>
                ...
                <button key={o.v} type="button" onClick={() => setSessionLen(o.v)}
                  className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold border-2 transition-all ${
                    sessionLen === o.v
                      ...
                      : 'border-[#e8e7f5] bg-white text-[#6B6A8A] hover:border-[#534AB7]/40'
                  }`}>
                  {o.e} {o.v}
                </button>
              ))}
          ...
          {/* ── Sleep window ─────────────────────────────────── */}
          <div className="bg-gradient-to-br from-[#1e1657] to-[#2d1f8a] rounded-2xl p-4 shadow-sm relative overflow-hidden">
            {/* Star decorations */}
            {[...Array(6)].map((_, i) => (
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/10 border border-white/20 rounded-xl p-3">
                  <p className="text-[10px] text-indigo-300 font-bold uppercase tracking-wide mb-1.5">
                    🌙 Bedtime
                  </p>
                  <input
                    className="w-full bg-transparent text-white font-extrabold text-lg outline-none text-center border-b border-white/30 pb-1 placeholder:text-white/40"
                    value={sleepStart}
                ...
                <div className="bg-white/10 border border-white/20 rounded-xl p-3">
                  <p className="text-[10px] text-amber-300 font-bold uppercase tracking-wide mb-1.5">
                    🌅 Wake up
                  </p>
                  <input
                    className="w-full bg-transparent text-white font-extrabold text-lg outline-none text-center border-b border-white/30 pb-1 placeholder:text-white/40"
          ...
          <div className="bg-white border border-[#e8e7f5] rounded-2xl p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-3">
              <Upload className="w-4 h-4 text-[#534AB7]" />
              <p className="text-sm font-bold text-[#1A1A2E]">Fixed weekly events</p>
              <span className="text-[10px] text-[#9B9AB5] ml-1 font-light">
            ...
            <div className="space-y-2 mb-3">
              {events.map((ev) => (
                <div key={ev.id}
                  className="flex items-center gap-3 bg-[#fafafe] border border-[#e8e7f5] rounded-xl px-3 py-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-0.5 mt-0.5 flex-wrap">
                      {ev.days.map((d) => (
                        <span key={d}
                          className="text-[9px] font-bold bg-[#534AB7]/10 text-[#534AB7] px-1.5 py-0.5 rounded-md">
                          {d[0]}
                        </span>
                      ))}
                      {ev.time && (
                        <span className="text-[9px] text-[#9B9AB5] ml-1">{ev.time}</span>
                      )}
                    </div>
                  </div>
                  <button
                    onClick={() => setEvents((p) => p.filter((e) => e.id !== ev.id))}
            ...
            {/* Add event form */}
            {adding ? (
              <div className="border-2 border-dashed border-[#534AB7]/30 rounded-xl p-3 space-y-2 bg-[#f5f3ff]/50">
                <div className="flex items-center gap-2">
                  <select value={addEmoji} onChange={(e) => setAddEmoji(e.target.value)}
                    className="text-xl bg-white border border-[#e8e7f5] rounded-xl px-2 py-1.5 outline-none cursor-pointer w-14 text-center">
                    {['📅','🏃','🚗','🏫','💪','🎵','🍽','🧘','📚','⚽'].map((e) => (
                  ...
                  <input
                    ...
                    placeholder="Event name (e.g. Gym, Lab, Commute)"
                  />
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <input
                    className={INP + ' w-24 text-xs py-2'}
                    value={addTime}
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setAdding(false)}
                    className="border border-[#e8e7f5] text-[#6B6A8A] font-semibold text-xs px-4 py-2 rounded-xl hover:bg-gray-50 transition-all">
                    Cancel
                  </button>
                </div>
              </div>
              ...
              <button onClick={() => setAdding(true)}
                className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-[#534AB7]/25 hover:border-[#534AB7]/50 text-[#534AB7] font-semibold text-xs py-3 rounded-xl transition-all hover:bg-[#f5f3ff]/50">
