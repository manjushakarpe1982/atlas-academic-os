...
export default function Screen5({ onNext, onBack, classId }: Props) {
  ...
  // ── Input styles ──
  const inp = "w-full px-2 py-1 text-xs border border-indigo-300 rounded-lg outline-none focus:border-indigo-500 bg-white";
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4 space-y-4" style={{ maxHeight: 420 }}>
                  ...
                  <div key={i} className="flex items-center gap-2 py-1.5 border-b border-gray-50 last:border-0">
                    {editGrades ? (
                      <>
                        ...
                        <input className={`${inp} flex-1`} value={w.category}
                          placeholder="Category"
                          onChange={e => setWeight(i, 'category', e.target.value)} />
                        <input className={`${inp} w-12`} type="number" min="0" max="100" value={w.weight_pct ?? ''}
                          placeholder="%"
                          onChange={e => setWeight(i, 'weight_pct', Number(e.target.value))} />
                        <span className="text-xs text-gray-400">%</span>
                        <button onClick={() => removeWeight(i)} className="text-red-400 hover:text-red-600">
          ...
          {(draft?.assessments?.length ?? 0) > 0 && (
            <div className="border border-gray-100 rounded-2xl overflow-hidden">
              ...
              <div className="px-4 py-2">
                {(showAllDates ? draft!.assessments : draft!.assessments.slice(0, 5)).map((a, i) => {
                  const badge = formatDateBadge(a.due_date);
                  return (
                    <div key={i} className="flex items-center gap-2 py-2 border-b border-gray-50 last:border-0">
                      {editDates ? (
                        <>
                          <input className={`${inp} flex-1`} value={a.title} placeholder="Title"
                            onChange={e => setAssessment(i, 'title', e.target.value)} />
                          <input className={`${inp} w-28`} type="date" value={a.due_date || ''}
                            onChange={e => setAssessment(i, 'due_date', e.target.value || null)} />
                          <button onClick={() => removeAssessment(i)} className="text-red-400 hover:text-red-600 flex-shrink-0">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </>
                      ) : (
                ...
                {!editDates && (draft?.assessments?.length ?? 0) > 5 && (
                  <button onClick={() => setShowAllDates(!showAllDates)}
                    className="flex items-center gap-1 text-xs text-indigo-600 font-semibold mt-2">
                    {showAllDates ? 'Show less' : `View all ${draft?.assessments?.length} dates`}
                    {showAllDates ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                  </button>
                )}
              </div>
            </div>
              ...
              <div className="px-4 py-2">
                {(showAllTopics ? draft!.topics : draft!.topics.slice(0, 3)).map((t, i) => (
                  <div key={i} className="flex items-center gap-2 py-1.5 border-b border-gray-50 last:border-0">
                    {editTopics ? (
                      <>
                        <input className={`${inp} w-14`} type="number" min="1" value={t.week_hint ?? ''}
                          placeholder="Wk"
                          onChange={e => setTopic(i, 'week_hint', e.target.value ? Number(e.target.value) : null)} />
                        <input className={`${inp} flex-1`} value={t.title} placeholder="Topic title"
