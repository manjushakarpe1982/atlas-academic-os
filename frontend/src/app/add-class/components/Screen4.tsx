...
export default function Screen5({ onNext, onBack, classId }: Props) {
  ...
  // ── Input styles ──
  const inp = "w-full max-w-full px-2 py-1 text-xs border border-indigo-300 rounded-lg outline-none focus:border-indigo-500 bg-white box-border";
  ...
  return (
    <Phone step={4} total={8}>
      <div className="flex flex-col bg-white min-h-[560px] w-full max-w-full overflow-hidden">

        {/* Header */}
        <div className="px-5 pt-4 pb-3 w-full max-w-full">
        ...
        {/* Accuracy badge */}
        <div className="mx-5 mb-3 flex items-center justify-between bg-green-50 border border-green-100 rounded-xl px-3 py-2 w-auto">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden px-5 pb-4 space-y-4 w-full max-w-full" style={{ maxHeight: 420 }}>
              ...
              {editCourse ? (
                <div className="flex items-center gap-2">
                  <button onClick={() => save(() => setEditCourse(false))}
                    className="flex items-center gap-0.5 text-xs text-green-600 font-bold">
              ...
              {editCourse ? (
                <div className="space-y-2">
                  {[
                    ...
                    <div key={f.field}>
                      <label className="text-[10px] font-bold text-gray-400 block mb-0.5">{f.label}</label>
                  ...
                  <div key={i} className="py-2 border-b border-gray-50 last:border-0 overflow-x-hidden w-full">
                    {editGrades ? (
                      <div className="flex items-center gap-2">
                        ...
                        <input 
                          className="flex-1 px-3 py-2 text-xs border border-gray-300 rounded-lg outline-none focus:border-indigo-500 bg-white"
                          value={w.category}
                          placeholder="Category name"
                          onChange={e => setWeight(i, 'category', e.target.value)} />
                        <input 
                          className="w-16 px-2 py-2 text-xs border border-gray-300 rounded-lg outline-none focus:border-indigo-500 bg-white text-center"
                          type="number" 
                          min="0" 
                          max="100" 
                          value={w.weight_pct ?? ''}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-gray-600 w-8 text-right flex-shrink-0">{w.weight_pct}%</span>
          ...
          {(draft?.assessments?.filter(a => !a.due_date)?.length ?? 0) > 0 && (
            <div className="border border-gray-100 rounded-2xl overflow-hidden">
              <div className="flex items-center justify-between px-3 py-2 bg-orange-50 border-b border-orange-100">
                <div className="flex items-center gap-1.5">
                  <span className="text-sm">📋</span>
                  <p className="text-[10px] font-extrabold text-gray-700 uppercase tracking-widest">Assessments</p>
                </div>
                {editDates ? (
                  <div className="flex items-center gap-2">
                    ...
                    {editDates ? (
                      <div className="flex items-center gap-2">
                        <input 
                          ...
                          onChange={e => setAssessment(idx, 'title', e.target.value)} />
                        <input 
                          className="w-24 px-2 py-2 text-xs border border-gray-300 rounded-lg outline-none focus:border-indigo-500 bg-white"
                          type="date" 
                          value={a.due_date || ''}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-gray-900 truncate">{a.title}</p>
                          <p className="text-[10px] text-gray-400">{a.due_date || 'TBD'}</p>
                        </div>
                        <ConfBadge level={a.confidence} />
                      </div>
                    )}
              ...
              <div className="px-4 py-2">
                {(showAllTopics ? draft!.topics : draft!.topics.slice(0, 3)).map((t, i) => (
                  <div key={i} className="py-2 border-b border-gray-50 last:border-0 overflow-x-hidden w-full">
                    {editTopics ? (
                      ...
                      <div className="flex items-center gap-2">
                        <p className="flex-1 text-xs font-semibold text-gray-800 min-w-0 truncate">
                ...
                {!editTopics && (draft?.topics?.length ?? 0) > 3 && (
                  <button onClick={() => setShowAllTopics(!showAllTopics)}
                    className="flex items-center gap-1 text-xs text-indigo-600 font-semibold mt-2">
