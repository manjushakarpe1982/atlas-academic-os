...
interface Props { grade: GradeItem; classId: string; onBack: () => void; onSaved: () => void; onDelete: () => void; }

export default function EditGradeForm({ grade, classId, onBack, onSaved, onDelete }: Props) {
  ...
  const handleSave = async () => {
    ...
    if (!maxScore || isNaN(Number(maxScore)) || Number(maxScore) <= 0) { setError('Enter a valid max score'); return; }

    setSaving(true);
    try {
      const token = getToken();
      const res = await fetch(`${API_BASE}/api/classes/${classId}/grades/${grade.id}`, {
        method: 'PATCH',
      ...
      if (!res.ok) { setError('Failed to update grade'); setSaving(false); return; }
      onSaved();
      ...
      <div className="px-4 py-4 space-y-4">
        <div><label className="text-xs font-bold text-gray-500 block mb-1.5">Assessment Name</label>
          <input value={title} onChange={e => setTitle(e.target.value)}
            className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:border-indigo-500 focus:outline-none" /></div>
        ...
        <div><label className="text-xs font-bold text-gray-500 block mb-1.5">Notes <span className="font-normal text-gray-400">(Optional)</span></label>
          <textarea placeholder="Add notes..." rows={3}
            className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm focus:border-indigo-500 focus:outline-none resize-none" /></div>
