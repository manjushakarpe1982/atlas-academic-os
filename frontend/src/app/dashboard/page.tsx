...
import { CheckCircle2, Circle, ChevronRight, ArrowRight, Brain, Loader2, Info, X } from 'lucide-react';
import { api } from '@/lib/api';

// ── Types ──
interface Summary { greeting: string; name: string; deadlines_this_week: number; high_priority_tasks: number; }
interface FocusTask { title: string; class_name: string; class_id: string; due_date: string | null; days_left: number | null; due_display: string; category: string; weight_pct: number | null; current_grade: number | null; priority: string; priority_score: number; confidence: string; source: string; reason: string; recommended_study_mins: number; }
interface PlanItem { class_id: string; class_name: string; mins: number; done: boolean; }
interface Deadline { id: string; title: string; class_name: string; due_date: string; due_display: string; day_name: string; category: string; priority: string; }
interface ClassGrade { id: string; name: string; term: string; grade: number | null; }
interface WeeklyProgress { sessions_done: number; sessions_goal: number; pct: number; }
interface DashboardData { summary: Summary; focusTask: FocusTask | null; todayPlan: PlanItem[]; upcomingDeadlines: Deadline[]; classGrades: ClassGrade[]; weeklyProgress: WeeklyProgress; aiRecommendation: string | null; }

// ── Helpers ──
const CLASS_COLORS = ['bg-green-500', 'bg-blue-500', 'bg-purple-500', 'bg-orange-500', 'bg-pink-500', 'bg-teal-500'];
const CLASS_LIGHT  = ['bg-green-100', 'bg-blue-100', 'bg-red-100', 'bg-orange-100', 'bg-pink-100', 'bg-teal-100'];
const CLASS_ICONS  = ['🌿', '📐', '⚗️', '📝', '🎨', '💻', '📜', '📖'];

function getClassStyle(i: number) {
  return { color: CLASS_COLORS[i % CLASS_COLORS.length], light: CLASS_LIGHT[i % CLASS_LIGHT.length], icon: CLASS_ICONS[i % CLASS_ICONS.length] };
}

function getCategoryIcon(cat: string): string {
  const c = (cat || '').toLowerCase();
  if (c.includes('quiz') || c.includes('exam') || c.includes('test')) return '📝';
...
function Tooltip({ lines }: { lines: string[] }) {
  ...
  return (
    <div className="relative inline-flex" ref={ref}>
      <button onClick={() => setOpen(!open)} className="text-gray-300 hover:text-indigo-500 transition-colors">
        <Info className="w-3.5 h-3.5" />
      </button>
      {open && (
        <div className="absolute left-0 top-6 z-50 w-72 bg-gray-900 text-white rounded-xl shadow-2xl p-3 text-xs leading-relaxed">
          <button onClick={() => setOpen(false)} className="absolute top-2 right-2 text-gray-400 hover:text-white">
            <X className="w-3 h-3" />
          </button>
          <div className="space-y-1.5 pr-4">
            {lines.map((line, i) => (
              <p key={i} className={line.startsWith('•') ? 'text-gray-300 pl-2' : line.startsWith('→') ? 'text-indigo-300 font-semibold' : 'text-white font-bold'}>
                {line}
              </p>
            ))}
          </div>
...
export default function DashboardHome() {
  ...
  const summary = data.summary || { greeting: 'Hello', name: 'Student', deadlines_this_week: 0, high_priority_tasks: 0 };
  ...
  const weeklyProgress = data.weeklyProgress || { sessions_done: 0, sessions_goal: 5, pct: 0 };
  ...
  return (
    <div className="px-4 py-4 space-y-5 pb-24">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-extrabold text-gray-900">{summary.greeting}, {summary.name} 👋</h1>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-1.5">
              <p className="text-xl font-extrabold text-gray-900">{summary.deadlines_this_week}</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-1.5">
              <p className="text-xl font-extrabold text-gray-900">{summary.high_priority_tasks}</p>
          ...
          <div className="bg-gradient-to-r from-white via-white/90 to-transparent">
            <div className="px-4 pt-3 pb-1">
              <div className="flex items-center gap-1.5 mb-2">
                <p className="text-[10px] font-extrabold text-indigo-500 uppercase tracking-widest">What to Study First</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-base font-extrabold text-gray-900">{focusTask.title}</p>
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      focusTask.priority === 'High' ? 'text-red-600 bg-red-50' :
                      focusTask.priority === 'Medium' ? 'text-amber-600 bg-amber-50' :
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    ...
                    {focusTask.weight_pct ? ` · Worth ${focusTask.weight_pct}%` : ''}
                    {focusTask.current_grade ? ` · Grade ${focusTask.current_grade}%` : ''}
                  </p>
                  <p className="text-xs text-gray-600 mt-1">Reason: {focusTask.reason}</p>
            ...
            {todayPlan.map((item, i) => {
              ...
              return (
                <div key={item.class_id} className="flex items-center gap-3">
                  <div className={`w-8 h-8 ${s.light} rounded-xl flex items-center justify-center flex-shrink-0`}>
          ...
          <div className="space-y-2">
            {upcomingDeadlines.slice(0, 3).map((d) => (
              <div key={d.id} className="flex items-center gap-3 py-1.5">
                <p className="text-xs text-gray-400 w-20 flex-shrink-0">{d.due_display}</p>
                ...
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${
                  d.priority === 'High' ? 'text-red-600 bg-red-50' : 'text-amber-600 bg-amber-50'
                }`}>
                  {d.priority}
                </span>
              </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {classGrades.slice(0, 4).map((c, i) => {
              ...
              return (
                <div key={c.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 flex flex-col items-center text-center">
                  <div className={`w-9 h-9 ${s.color} rounded-xl flex items-center justify-center text-base mb-1.5`}>{s.icon}</div>
                  <p className="text-[9px] font-bold text-gray-700 leading-tight mb-1 truncate w-full">{c.name}</p>
                  <p className={`text-sm font-extrabold ${gradeColor(c.grade)}`}>{c.grade !== null ? `${c.grade}%` : '—'}</p>
                  <div className="w-full h-1 bg-gray-100 rounded-full mt-1.5 overflow-hidden">
                    <div className={`h-full ${s.color} rounded-full`} style={{ width: `${grade}%` }} />
                  </div>
                </div>
              );
            })}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <div className="flex items-center gap-1.5 mb-4">
          <h2 className="text-base font-extrabold text-gray-900">Weekly Progress</h2>
        ...
        <div className="flex items-center gap-5">
          <div className="relative w-20 h-20 flex-shrink-0">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-base font-extrabold text-purple-700">{weeklyProgress.pct}%</span>
            </div>
          </div>
          <div className="flex-1">
            <p className="text-3xl font-extrabold text-gray-900 leading-none">
              {weeklyProgress.sessions_done} <span className="text-xl font-bold text-gray-400">/ {weeklyProgress.sessions_goal}</span>
            </p>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <p className="text-xs font-bold text-purple-700">{weeklyProgress.sessions_done} of {weeklyProgress.sessions_goal} sessions</p>
          </div>
        ...
        <div className="border-t border-gray-100 mt-4 pt-3">
          <p className="text-xs text-gray-400">Goal: {weeklyProgress.sessions_goal} study sessions this week</p>
        </div>
      </div>
        ...
        <div className="bg-indigo-50 border border-indigo-100 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 mb-0.5">
              <p className="text-[10px] font-extrabold text-indigo-500 uppercase tracking-widest">Atlas Recommendation</p>
              <Tooltip lines={TIPS.aiRecommendation} />
            </div>
            <p className="text-sm text-gray-700 leading-relaxed">{aiRecommendation}</p>
          </div>
