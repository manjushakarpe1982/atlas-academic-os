...
import {
  Check, Shield, HelpCircle,
  ArrowLeft, ArrowRight, LayoutDashboard,
  AlertTriangle, X,
} from 'lucide-react';
import { STEPS } from './constants';

interface Props { children: React.ReactNode; step: number; disableNext?: boolean; }

/* ─── Skip modal ─────────────────────────────────────────────── */
function SkipModal({ onClose, onDashboard }: { onClose:()=>void; onDashboard:()=>void }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[28px] shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-8">
          ...
          <h2 className="text-2xl font-extrabold text-[#1A1A2E] mb-2">Skip setup?</h2>
          ...
          <div className="bg-[#F8F7FF] border border-[#E8E5FD] rounded-2xl p-4 mb-6">
            <p className="text-[11px] font-extrabold text-[#534AB7] uppercase tracking-widest mb-3">What you&apos;ll miss</p>
            <ul className="space-y-2">
              {['Personalised study plan based on your goals','AI study guides calibrated to your field','Sleep & schedule conflict detection','Accurate grade predictions for your courses'].map((item) => (
...
export default function OnboardingLayout({ children, step, disableNext }: Props) {
  ...
  return (
    <>
      {showSkip && <SkipModal onClose={() => setShowSkip(false)} onDashboard={() => router.push('/home')} />}
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0">
            ...
            <div className="md:hidden bg-white border-b border-[#EEEDFE] px-4 py-3 flex-shrink-0">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-extrabold text-[#1A1A2E]">Step {step} of {STEPS.length}</span>
                <span className="text-xs text-[#9B9AB5]">{STEPS[step-1]?.label}</span>
              </div>
              <div className="h-1.5 bg-[#EEEDFE] rounded-full overflow-hidden">
                <div className="h-full bg-[#534AB7] rounded-full transition-all duration-500" style={{ width:`${pct}%` }} />
              </div>
            </div>
